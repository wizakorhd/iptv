...
### Auto‑refresh model
The **local macOS `launchd` job is the primary builder** — it rebuilds and pushes
from your India connection (see setup below). It revalidates the **playlist daily**
(~90 s) and re‑grabs the **3‑day EPG only when the current guide has ≤1 day left**
(~every 2 days, ~40 min) so most runs are fast. It also `--refresh`es the iptv‑org
source snapshot weekly so newly added/removed channels are picked up.

- **GitHub Actions are manual‑only failsafes** (schedules disabled to avoid
  double‑push churn with the local job): trigger `refresh-epg.yml` from the
  Actions tab to rebuild the EPG on GitHub's runners when your Mac is offline for
  a while, or `refresh-playlist.yml` if you prefer an Actions flow with a
  registered self‑hosted India runner. Both publish only `guide.xml.gz`.
